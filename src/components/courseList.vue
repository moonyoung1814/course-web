<template>
    <div class="ring-2 rounded shadow w-10/12 ml-12 relative flex flex-col px-8">
        <label class="text-2xl font-bold self-center mt-8">创新实践{{isPractice}}</label>
        <div class="grid grid-cols-2 gap-x-8 w-full mt-4">
            <div v-for="(item,index) in course" :key="index" class="w-full"> 
                <div class="flex flex-col w-full" v-if="item.name == '创新实践' + isPractice">
                    <div class="flex flex-row mt-4 ">
                        <img :src="item.teacher.image" alt="head" class="ring-2 ring-gray-300 shadow w-8 h-8 rounded-full overflow-hidden">
                        <p class="text-sm ml-4 self-center font-bold">{{item.teacher.name}}</p>
                    </div>
                    <!-- 学期卡片 -->
                    <div class="flex flex-row mt-4 space-x-4 w-full">
                        <div class="ring-2 ring-gray-300 shadow rounded p-2 flex flex-row">
                            <p class="text-xs">第</p>
                    ...
                    <div class="flex flex-row mt-4 w-full">
                        <div class="flex flex-col ring-2 ring-gray-300 rounded shadow-md p-4 w-full">
                            <div class="flex flex-row items-center">
                                <label class="iconfont icon-thin-_book_writi text-blue-600 text-base font-bold"></label>
                                <label class="text-sm text-black ml-2 font-bold">课程教材</label>
                            </div>
                            ...
                            <div class="flex flex-row items-center mt-3">
                                <label class="iconfont icon-thin-_book_writi text-blue-600 text-base font-bold"></label>
                                <label class="text-sm text-black ml-2 font-bold">参考书目</label>
                            </div>
